...
export function OOGlow({ width = 300, height = 120, className = "", pulse = true }: OOGlowProps) {
  const pathname = usePathname();
  const reduced = useReducedMotion();
  const color = getColor(pathname);
  ...
  return (
    <motion.svg
      ...
      animate={
        pulse && !reduced
          ? {
              filter: [
                `drop-shadow(0 0 6px ${color}) drop-shadow(0 0 18px ${color}80) drop-shadow(0 0 40px ${color}40)`,
                `drop-shadow(0 0 12px ${color}) drop-shadow(0 0 32px ${color}aa) drop-shadow(0 0 64px ${color}60)`,
                `drop-shadow(0 0 6px ${color}) drop-shadow(0 0 18px ${color}80) drop-shadow(0 0 40px ${color}40)`,
              ],
            }
          : { filter: `drop-shadow(0 0 8px ${color}) drop-shadow(0 0 24px ${color}80)` }
      }
      transition={
        pulse && !reduced
    ...
    >
      <g transform="translate(12 28)">
        <path d={O_PATH_1} fill={color} />
      </g>
      <g transform="translate(362 36)">
        <path d={O_PATH_2} fill={color} />
      </g>
    </motion.svg>
  );
}