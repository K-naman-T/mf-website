...
export type LogoPfpVariant = "bicolor" | "red" | "white" | "dark" | "black-on-red";
export type LogoPfpShape = "circle" | "squircle" | "square";
export type LogoPfpLayout = "horizontal" | "stacked" | "composite";
export type LogoPfpBg = "black" | "white" | "red" | "transparent";

export interface LogoPfpProps {
  variant?: LogoPfpVariant;
  layout?: LogoPfpLayout;
  shape?: LogoPfpShape;
  bg?: LogoPfpBg;
  size?: number | string;
...
// Horizontal: 380px target width in 512x512
const S_HORIZ = 380.0 / 647.4307;
const DX_HORIZ = 256.0 - 335.715 * S_HORIZ; // ~58.958
const DY_HORIZ = 256.0 - 138.155 * S_HORIZ; // ~174.912

// Stacked: scale 0.72 in 512x512
const S_STACK = 0.72;
const TOP_X = 256.0 - (335.0 * S_STACK) / 2.0;
const TOP_Y = 256.0 - 326.7 / 2.0;
const BOT_X = 256.0 - (298.0 * S_STACK) / 2.0;
const BOT_Y = TOP_Y + 221.0 * S_STACK + 20.0;

// Composite: scale 0.4976 in 512x512
const S_COMP = 205.0 / 412.0;
const DX_COMP = 256.0 - 352.5 * S_COMP;
const DY_COMP = 256.0 - 263.0 * S_COMP;
...
const BG_COLOR_MAP: Record<LogoPfpBg, string> = {
  black: "#000000",
  white: "#FFFFFF",
  red: "#FF0000",
...
export function LogoPfp({
  ...
  showSafeZone = false,
  className,
  "aria-label": ariaLabel = "OddFromYou Profile Picture",
}: LogoPfpProps) {
  const palette = COLOR_MAP[variant];
  const activeBg = bg ?? palette.defaultBg;
  const bgColor = BG_COLOR_MAP[activeBg];
  ...
  return (
    <div
      className={cn(
        "relative overflow-hidden inline-flex items-center justify-center select-none flex-shrink-0",
        shapeClass,
        className,
      )}
      style={sizeStyle}
    >
      ...
      >
        ...
        {layout === "horizontal" && (
          <g transform={`translate(${DX_HORIZ.toFixed(4)} ${DY_HORIZ.toFixed(4)}) scale(${S_HORIZ.toFixed(6)})`}>
            <path d={D_LEFT} fill={palette.left} transform="translate(12 28)" />
            <path d={D_RIGHT} fill={palette.right} transform="translate(362 36)" />
          </g>
        )}

        {layout === "stacked" && (
          <>
            <g transform={`translate(${TOP_X.toFixed(4)} ${TOP_Y.toFixed(4)}) scale(${S_STACK.toFixed(4)})`}>
              <path d={D_LEFT} fill={palette.left} />
            </g>
            <g transform={`translate(${BOT_X.toFixed(4)} ${BOT_Y.toFixed(4)}) scale(${S_STACK.toFixed(4)})`}>
              <path d={D_RIGHT} fill={palette.right} />
            </g>
          </>
        )}

        {layout === "composite" && (
          <g transform={`translate(${DX_COMP.toFixed(4)} ${DY_COMP.toFixed(4)}) scale(${S_COMP.toFixed(6)})`}>
            <g transform="translate(12 28)">
              <path d={D_LEFT} fill={palette.left} fillRule="evenodd" />
            </g>
            <g transform="translate(362 36)">
              <path d={D_RIGHT} fill={palette.right} fillRule="evenodd" />
            </g>
            <path d={PATH_O} fill={palette.letterO} fillRule="evenodd" />
            <path d={PATH_F} fill={palette.letterF} />
            <path d={PATH_Y} fill={palette.letterY} />
          </g>
        )}
